...
def test_update_user_while_not_logged_in(client: TestClient):
    ...
    response = client.put("/user/update", json={"username": "test_update", "email": "test_update", "password": "test_update"})
    assert response.status_code == 401
    
def test_update_user(client: TestClient):
    client.post("/user/create", json={"username": "chat", "email": "chat", "password": "patate"})
    login_response = client.post("/login", data={"username":"chat", "password":"patate"})
